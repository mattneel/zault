...
export class Zault {
    ...
    constructor(instance) {
        ...
        this.#chacha20Overhead = instance.exports.zault_get_chacha20_overhead();
        this.#chacha20KeyLen = instance.exports.zault_get_chacha20_key_len();
    }
     ...
     */
    generateGroupKey() {
        return this.randomBytes(this.#chacha20KeyLen);
    }

    /**
     * Encrypt data with ChaCha20-Poly1305 using a pre-shared key.
     * Use this for group messages where the group key is already distributed.
     * @param {Uint8Array} key 32-byte symmetric key
     * @param {string|Uint8Array} plaintext Data to encrypt
     * @returns {Uint8Array} Ciphertext (28 bytes overhead)
     */
    encryptWithKey(key, plaintext) {
        if (key.length !== this.#chacha20KeyLen) {
            throw new ZaultError(ZAULT_ERR_INVALID_ARG, 'Key must be 32 bytes');
        }
        ...
        const keyPtr = this.#write(key);
        const ptPtr = this.#write(plaintextBytes);
        const ctLen = plaintextBytes.length + this.#chacha20Overhead;
        const ctPtr = this.#alloc(ctLen);

        const result = this.#instance.exports.zault_chacha20_encrypt(
            keyPtr, key.length,
            ptPtr, plaintextBytes.length,
            ctPtr, ctLen
        );
        if (result !== ZAULT_OK) {
            throw new ZaultError(result, 'Failed to encrypt with key');
        }
        return this.#read(ctPtr, ctLen);
    }
    ...
    decryptWithKey(key, ciphertext) {
        ...
        if (ciphertext.length < this.#chacha20Overhead) {
            throw new ZaultError(ZAULT_ERR_INVALID_ARG, 'Ciphertext too short');
        }

        const keyPtr = this.#write(key);
        const ctPtr = this.#write(ciphertext);
        const ptLen = ciphertext.length - this.#chacha20Overhead;
        const ptPtr = this.#alloc(ptLen);

        const result = this.#instance.exports.zault_chacha20_decrypt(
            keyPtr, key.length,
            ctPtr, ciphertext.length,
            ptPtr, ptLen
        );
        if (result !== ZAULT_OK) {
            throw new ZaultError(result, 'Failed to decrypt with key');
        }
        return this.#read(ptPtr, ptLen);
    }
     ...
     * @returns {string}
     */
    decryptWithKeyString(key, ciphertext) {
        return new TextDecoder().decode(this.decryptWithKey(key, ciphertext));
    }
    ...
    get CHACHA20_OVERHEAD() { return this.#chacha20Overhead; }
    get CHACHA20_KEY_LEN() { return this.#chacha20KeyLen; }
